import { Directive, ElementRef, Input, SimpleChange, HostListener, ChangeDetectorRef, OnInit, AfterViewInit, numberAttribute } from '@angular/core';
...
export class NsAutoHeightTableDirective implements OnInit, AfterViewInit {
  @Input({ transform: numberAttribute, alias: 'nsAutoHeightTable' })
  offset: number = 1;
  ...
  private doAutoSize = () => {
    ...
    setTimeout(() => {
      ...
      if (this.element && this.element.nativeElement && this.element.nativeElement.parentElement && this.element.nativeElement.parentElement.offsetHeight) {
